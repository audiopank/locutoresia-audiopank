/**
 * static/loudness.js — PORTA de minidaw-react/src/lib/loudness.js pra MiniDAW
 * clássica (16/09/2026, Suíte Master módulo A). Mesma matemática (BS.1770-4,
 * pico real por Catmull-Rom), sem ES modules: vira `window.Loudness` no
 * navegador e `module.exports` no node (tests/test_master_suite.py).
 *
 * ⚠️ NÃO edite a conta aqui: edite a fonte React e rode de novo o script de
 * porta (scratchpad/portar_loudness.py). Só o rodapé (medir/canaisDe) é daqui.
 */
(function (global) {
'use strict';
 ...
 */
function blocosDeLoudness(canais, sr) {
 ...
 */
function lufsIntegrado(canais, sr) {
...
function picoRealDbTodos(canais, sr) {
 ...
 */
function faixaDinamica(canais, sr) {
 ...
 */
function balancoTonal(canais, sr) {
...
/** Extrai os canais de um AudioBuffer como Float32Array[] (de mastering.js). */
function canaisDe(buffer) {
    const out = [];
    for (let c = 0; c < buffer.numberOfChannels; c++) out.push(buffer.getChannelData(c));
    return out;
}

/** Medição completa de um AudioBuffer: LUFS integrado, pico real, faixa dinâmica, balanço. */
...
const Loudness = { coefShelfAgudo, coefPassaAlta, biquad, filtroK, blocosDeLoudness, lufsIntegrado, picoRealDb, picoRealDbTodos, faixaDinamica, BANDAS, balancoTonal, misturarMono, canaisDe, medir };
