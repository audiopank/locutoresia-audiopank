...
e o TOM fica o mesmo (WSOLA em static/time-stretch.js). Voz usa o perfil 'voz' (20 ms); trilha e efeito
usam o perfil 'musica' (50 ms) — "às vezes o produtor precisa dar uma apressadinha na trilha".
...
def test_motor_encurta_sem_mudar_o_tom():
    ...
    assert '# fail 0' in r.stdout and '# pass 8' in r.stdout


def test_alca_em_toda_faixa_com_perfil_por_tipo_e_prioridade_sobre_trim():
    js = _ler('static', 'minidaw.js')
    assert "alca.className = 'clip-alca-stretch';" in js and "alca.dataset.stretch = '1';" in js
    assert "if (track.type === 'voice' && window.TimeStretch) {" not in js          # trilha e efeito também ganham a alça
    assert "return track.type === 'voice' ? 'voz' : 'musica';" in js                # perfil pelo tipo da faixa
    assert "'Time Stretch: arraste pra apressar/desacelerar a trilha sem mudar o tom" in js
    ...
    assert "rotulo.classList.toggle('forcado', !TimeStretch.soaNatural(fator, perfil));" in js
...
def test_estica_a_partir_da_origem_e_entra_no_desfazer():
    ...
    assert "TimeStretch.esticarBuffer(this.audioContext, base.buffer, base.offset, base.duracao, fator, perfil)" in trecho
    assert "' Em trilha, acima de ~15% o ritmo começa a borrar" in trecho          # aviso próprio da trilha
...
def test_tela_carrega_o_motor_antes_da_minidaw(cliente):
    ...
    for t in ('.clip-alca-stretch {', '.clip-stretch-rotulo {', '.clip-stretch-rotulo.forcado {',
              'time-stretch.js?v=2', 'minidaw.js?v=67'):
        assert t in html, t
    assert html.index('clip-model.js?v=5') < html.index('time-stretch.js?v=2') < html.index('minidaw.js?v=67')
