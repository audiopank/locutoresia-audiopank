...
test('paramsDeesser: forca sobe, threshold desce, ratio sobe; makeup anulado; lixo = bypass', () => {
    ...
    assert.equal(p5.threshold, -30);
    assert.equal(p5.ratio, 6);
    assert.equal(p5.knee, 0);                                          // joelho 0: a conta do makeup fecha
    assert.ok(Math.abs(p5.compDb - 0.6 * -30 * (1 - 1 / 6)) < 1e-9);  // -15 dB anula o makeup automático
...
test('criarDeesser: crossover LR4 em 5 kHz (2+2 biquads Q=1/√2), alta no compressor, baixa intacta, desligado = seco', () => {
    ...
    assert.equal(de.comp.threshold.value, -30);
    assert.equal(de.comp.ratio.value, 6);
...
test('forcaDeesserDaFaixa: projeto antigo sem o campo = 5', () => {
